...
function completeEvidence(value) {
  return Boolean(
    value && value.experimentId && value.inputSnapshotId && value.candidate?.model
      && value.candidate?.solver && value.candidate?.version
      && Number.isFinite(value.objective) && typeof value.feasible === 'boolean'
      && Number.isFinite(value.runtimeMs) && Array.isArray(value.provenance) && value.provenance.length >= 2
  );
}
...
export async function runSolverCandidates({ adapters, input, timeoutMs = 5000, validation = {} } = {}) {
  ...
  const settled = await Promise.all(adapters.map(async (adapter, index) => {
    ...
    try {
      const result = await Promise.race([
        ...
        new Promise((_, reject) => { timer = setTimeout(() => reject(new Error(`solver timeout after ${timeoutMs}ms`)), timeoutMs); }),
      ]);
      clearTimeout(timer);
      object(result, `${label} evidence`);
      const evidence = Object.freeze({ ...result, runtimeMs: Number.isFinite(result.runtimeMs) ? result.runtimeMs : Date.now() - started });
      return { index, evidence, failure: null };
    } catch (error) {
      clearTimeout(timer);
      return { index, evidence: null, failure: Object.freeze({ adapter: label, timeout: /timeout/i.test(error.message), reason: error.message }) };
    }
  }));

  const evidences = settled.filter(({ evidence }) => evidence !== null).map(({ evidence }) => evidence);
  ...
  const eligible = indexed
    .filter(({ evidence }) => completeEvidence(evidence) && evidence.feasible === true && evidence.timeout === false)
  ...
  const promotion = primary
    ...
    : Object.freeze({ status: 'rejected', authoritative: false, checks: { evidenceComplete: false, feasible: false, ...validationState }, reason: 'no candidate produced complete promotable evidence' });

  return Object.freeze({
    runnerVersion: RUNNER_VERSION,
    candidates: comparison,
    failures,
    selectedFingerprint: primaryComparisonIndex >= 0 ? comparison[primaryComparisonIndex].fingerprint : null,
    ranking: eligible.map(({ evidence }) => {
      const comparisonIndex = evidences.indexOf(evidence);
      return { rank: eligible.findIndex((entry) => entry.evidence === evidence) + 1, fingerprint: comparison[comparisonIndex].fingerprint, objective: evidence.objective, runtimeMs: evidence.runtimeMs };
    }),
    promotion,
    authoritative: false,
  });
