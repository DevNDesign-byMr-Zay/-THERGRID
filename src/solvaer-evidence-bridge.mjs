...
export function fingerprintSolvaerEvidence(evidence) {
  return createHash('sha256')
    .update(JSON.stringify(canonical(evidence)), 'utf8')
    .digest('hex');
}

export function createSolvaerEvidenceBridge({
  request,
  candidate,
  provenanceRef,
  simulation,
  ...
  if (!decisionReceipt || typeof decisionReceipt !== 'object' || Array.isArray(decisionReceipt))
    throw new TypeError('decisionReceipt must be an object');
  const body = {
    contractVersion: 1,
...
export function validateSolvaerEvidenceBridge(evidence) {
  ...
  if (
    ...
    body.safety?.actuatesHardware !== false ||
