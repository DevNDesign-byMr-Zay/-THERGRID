...
export function simulateProposal({ twinState, proposal, durationMinutes = 15 } = {}) {
  ...
  if (decision.snapshotId !== twin.snapshotId) throw new TypeError('proposal.snapshotId must match twinState.snapshotId');
  if (decision.advisoryOnly !== true) throw new TypeError('proposal must remain advisory-only');
  if (!Number.isInteger(durationMinutes) || durationMinutes <= 0) throw new TypeError('durationMinutes must be a positive integer');
