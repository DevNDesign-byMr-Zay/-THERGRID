...
export function buildAdversarialCase(name, snapshot, { now = Date.now() } = {}) {
  ...
  switch (name) {
    ...
    case 'fallback-activation':
      metadata = { fallback: 'classical-reference-v1', fallbackUsed: true, primaryStatus: 'unavailable' };
      break;
    case 'seed-drift':
...
function evaluateSolverFixture(testCase) {
  ...
  const base = {
    ...
    provenance: [testCase.fingerprint],
  };
  const evidence = buildSolverEvidence(base);
  const promotion = evaluatePromotionGate({
    evidence,
    simulationPassed: evidence.feasible,
    receiptValid: true,
...
export function evaluateAdversarialCase(testCase, { now = Date.now() } = {}) {
  ...
  try {
    switch (value.name) {
      case 'stale-telemetry': {
        ...
        reason = accepted ? 'telemetry is within freshness policy' : 'telemetry exceeds freshness policy';
      ...
      case 'fallback-activation':
        accepted = value.metadata.fallbackUsed === true && value.metadata.fallback;
        reason = accepted ? 'fallback identity explicitly recorded' : 'fallback identity missing';
        break;
      case 'seed-drift': {
        const first = buildSolverEvidence({
          experimentId: 'adversarial-seed-drift', inputSnapshotId: value.snapshot.snapshotId,
          candidate: { model: 'test-candidate', solver: 'test-solver', version: 'v1' },
          seed: value.metadata.seed, objective: 1, feasible: true, runtimeMs: 1,
        });
        const second = buildSolverEvidence({ ...first, seed: value.metadata.alternateSeed });
        ...
        reason = accepted ? 'seed drift was not captured by evidence fingerprint' : 'seed drift produces distinct evidence fingerprints';
  ...
  const expectedRejection = new Set(ADVERSARIAL_FIXTURES).has(value.name) && value.name !== 'fallback-activation';
  const safe = expectedRejection ? !accepted : accepted;
  return Object.freeze({
    version: EVALUATION_VERSION,
    fixture: value.name,
    accepted,
