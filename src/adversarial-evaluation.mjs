...
export function buildAdversarialCase(name, snapshot, { now = Date.now() } = {}) {
  ...
  switch (name) {
    case 'stale-telemetry':
      candidate.observedAt = new Date(
        now - DEFAULT_MAX_TELEMETRY_AGE_MS - 1,
      ).toISOString();
  ...
  try {
    switch (value.name) {
      case 'stale-telemetry': {
        ...
        accepted =
          ageMs <=
          (value.metadata.maxAgeMs ?? DEFAULT_MAX_TELEMETRY_AGE_MS);
        reason = accepted
      ...
      case 'fallback-activation':
        accepted =
          value.metadata.fallbackUsed === true &&
          Boolean(value.metadata.fallback);
        reason = accepted
          ? 'fallback identity explicitly recorded'
