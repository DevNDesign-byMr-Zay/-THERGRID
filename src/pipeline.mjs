...
export function runSyntheticMicrogrid(snapshot) {
  ...
  const provenanceValid = validateProvenanceGraph(provenance, {
    requiredTypes: ['telemetry', 'twin-state', 'forecast', 'operating-proposal', 'simulation', 'decision-receipt', 'spatial-scene'],
  });
  const solverEvidence = buildSolverEvidence({
    ...
    feasible: simulation.status === 'passed' && Math.abs(simulation.outputs.residualBalanceKw) <= 0.000001,
