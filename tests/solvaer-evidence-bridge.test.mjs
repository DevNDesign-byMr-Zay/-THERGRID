...
import { createSolvaerEvidenceBridge, validateSolvaerEvidenceBridge } from '../src/solvaer-evidence-bridge.mjs';
...
test('creates verifiable SOLVÆR evidence after simulation', () => {
  ...
  const handoff = evaluateSolvaerDecisionHandoff({
    ...
    candidate: { experimentId: baseline.experimentId, snapshotId: fixture.snapshotId, proposal: baseline.proposal },
...
test('rejects tampered SOLVÆR evidence', () => {
  ...
  assert.equal(validateSolvaerEvidenceBridge({ ...evidence, safety: { authoritative: true, actuatesHardware: false, advisoryOnly: true } }), false);
});
