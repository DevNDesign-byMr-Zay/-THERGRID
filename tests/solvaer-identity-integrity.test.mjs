...
test('rejects a candidate identity swap before evidence is admitted', () => {
  ...
  const swapped = {
    ...candidate,
    producerIdentity: { ...SOLVAER_PRODUCER_IDENTITY, family: 'VÆLON' },
  };
  assert.equal(
    validateSolvaerCollaborationResult({
      request: baseline.solvaerRequest,
...
test('rejects a provenance identity swap even when the candidate is unchanged', () => {
  const { baseline, candidate, provenanceRef } = baselineArtifacts();
  const swappedProvenance = {
    ...provenanceRef,
    producerIdentity: { ...SOLVAER_PRODUCER_IDENTITY, family: 'ROARY' },
  };
  assert.equal(
    validateSolvaerCollaborationResult({
      request: baseline.solvaerRequest,
      candidate,
      provenanceRef: swappedProvenance,
    }),
    false,
  );
});
...
test('rejects a request identity swap rather than silently treating it as SOLVÆR', () => {
  ...
  const swappedRequest = {
    ...baseline.solvaerRequest,
    producerIdentity: { ...SOLVAER_PRODUCER_IDENTITY, family: 'AUREN' },
  };
  assert.equal(
    validateSolvaerCollaborationResult({
