...
test('normalizes a safe SOLVÆR proposal for simulation', () => {
  const request = createSolvaerOptimizationRequest({
    experimentId: 'exp-1', snapshotId: 'snap-1', twinStateRef: 'twin-state:snap-1',
    objective: 'reduce residual balance', constraints: { maxDeltaKw: 5 },
  });
  const result = validateSolvaerCollaborationResult({
    request,
    candidate: { experimentId: 'exp-1', snapshotId: 'snap-1', proposal: { dispatchKw: 2 }, objective: 'reduce residual balance' },
...
test('rejects a candidate that attempts physical authority', () => {
  const request = createSolvaerOptimizationRequest({ experimentId: 'exp-2', snapshotId: 'snap-2', twinStateRef: 'twin-state:snap-2', objective: 'explore' });
  assert.throws(() => validateSolvaerCollaborationResult({
    request,
