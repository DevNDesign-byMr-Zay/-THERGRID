...
test('chains sealed operator attention after collaboration evidence in provenance', () => {
  ...
  const graph = buildProvenanceGraph({
    ...
    collaborationEvidence: bridge.collaborationEvidence,
    operatorAttention: bridge.operatorAttention,
  });
  assert.equal(
    validateProvenanceGraph(graph, {
      requiredTypes: ['render-packet', 'solvaer-collaboration', 'operator-attention'],
    }),
    true,
  );
  assert.deepEqual(graph.nodes.map((node) => node.type).slice(-3), [
    'render-packet',
  ...
  assert.equal(
    attentionNode.id,
    `operator-attention-${bridge.operatorAttention.attentionFingerprint.slice(0, 16)}`,
  );
...
test('rejects tampered or unsealed operator attention before provenance projection', () => {
  ...
  const tampered = {
    ...
    items: [
      { ...bridge.operatorAttention.items[0], priority: 99 },
      bridge.operatorAttention.items[1],
    ],
  };
  assert.throws(
    () =>
...
test('provenance validation rejects substituted operator attention fingerprint identity', () => {
  ...
  assert.equal(
    validateProvenanceGraph({ ...graph, nodes }, { requiredTypes: ['operator-attention'] }),
    false,
  );
});
