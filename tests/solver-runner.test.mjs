...
const input = {
  ...
  seed: 11,
  provenance: ['receipt-runner-001', 'scene-runner-001'],
};
...
test('runner uses runtime and adapter order as deterministic tie-breakers', async () => {
  const makeAdapter = (model, objective, runtimeMs) =>
    Object.freeze({
      ...
      solve() {
        return {
          ...
          candidate: { model, solver: `${model}-solver`, version: 'v1' },
          ...
          timeout: false,
          fallback: null,
          provenance: ['receipt-001', 'scene-001'],
        };
      },
    });
  const result = await runSolverCandidates({
    adapters: [makeAdapter('slow', 1, 8), makeAdapter('fast', 1, 2), makeAdapter('first', 1, 2)],
    input,
    validation: { simulationPassed: true, receiptValid: true, provenanceValid: true },
  });
...
test('runner does not self-authorize when validation gates are absent', async () => {
  const result = await runSolverCandidates({
    adapters: [createClassicalReferenceAdapter()],
    input,
  });
  assert.equal(result.promotion.status, 'rejected');
