export function createOptimizationReceipt({ problem, result, startedAt, durationMs } = {}) {
  ...
  if (durationMs !== undefined && (!Number.isFinite(durationMs) || durationMs < 0)) throw new TypeError('durationMs must be non-negative.');

  return {
    schema: 'thergrid-optimization-receipt-v1',
    problem: { kind: problem.kind, version: problem.version, variableCount: problem.linear.length },
    solver: {
