...
async function main() {
  ...
  for (const key of ['PORT', 'SERVICE_NAME', 'THERGRID_PORT', 'THERGRID_LOG_LEVEL', 'GITHUB_SHA', 'RELEASE_TAG']) {
  ...
  assert(/docker compose -f docker-compose\.yml config --quiet/u.test(ci), 'CI must validate canonical docker-compose.yml');
